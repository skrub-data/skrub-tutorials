...
We want to build a simple model as baseline to decide whether there is enough of
a signal in our data to build on. 
...
## Step 3: Impute missing values {.smaller auto-animate="true"}
...
## Step 3: Impute missing values {auto-animate="true"}

::: {.callout-important}

Missing values are important! The fact that a value is missing is by itself useful 
...
A dataset about **employment** has the column **current position**. A missing value
here likely indicates **unemployment**, and imputing it with "most frequent" would
lead to incorrect assumptions about the sample. 

## Step 4: Scale numerical features {.smaller}
Some models work better when all the numeric features have comparable scale. 

`RidgeCV` is one of them, so we need to scale numerical features.
...
Train a `RidgeCV` regression model and evaluate with cross-validation.
...
## Intermission: data leakage and fit/transform {auto-animate="true"}
...
![](images/fit-transform-impute-1.png)
...
The proper way to do it: fit the transformer on the training data, transform 
the training with the same mean:
...
![](images/fit-transform-impute-2.png)
![](images/fit-transform-impute-3.png)
...
## What's in the `tabular_pipeline`
![](images/tabular-pipeline.png){fig-align="center"}

## Pre-processing is part of the pipeline! {.smaller}

There isn't a clear separation between "pre-processing", "feature engineering"
and "learning".

If you learn something while pre-processing your training data
("this is a datetime"), you need to apply that knowledge to the test data. 

This is why skrub provides **transformers**, rather than **functions**: transformers
are stateful and can be used in pipelines, functions are not. 

## What we saw in this chapter
- We built a predictive pipeline using traditional tools
- We talked about missing data imputation
- We discussed data leakage
- We tested skrub's `tabular_pipeline`
- We covered why pre-processing is part of the pipeline
