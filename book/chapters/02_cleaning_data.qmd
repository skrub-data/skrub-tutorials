...
In this chapter, we will show how we can quickly address this task using skrub's 
`Cleaner`. 
...
- The `Cleaner` object automates common data cleaning steps, making it easy to
prepare tabular data for analysis or modeling with minimal configuration.
- `Cleaner` leverages transformers like `DropUninformative` to automatically
remove columns that are empty, constant, or contain mostly unique values (such
as IDs), based on user-defined thresholds.
- The `DropUninformative` transformer can also be used directly for fine-grained
control over which columns to drop, according to missing value fraction, constant
